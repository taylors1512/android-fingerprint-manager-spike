package com.example.biometricsonly3.activity
...
import com.example.biometricsonly3.databinding.ActivityMainBinding
import com.example.biometricsonly3.architecuture.BaseActivity
import com.example.biometricsonly3.architecuture.observe
import com.example.biometricsonly3.fingerprintPrompt.FingerprintAuthenticationDialogFragment
import com.example.biometricsonly3.security.PreSetupChecker
import com.example.biometricsonly3.security.PreSetupChecks
import com.example.biometricsonly3.security.credentials.SecurityChecks
...
class MainActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.preSetupChecker = PreSetupChecker(this@MainActivity.applicationContext)
    ...
    private fun showBiometricPromptForRegistration(cipher: Cipher) {
        FingerprintAuthenticationDialogFragment().apply {
            setCryptoObject(FingerprintManagerCompat.CryptoObject(cipher))
            setCallback(object : FingerprintAuthenticationDialogFragment.Callback {
                override fun onAuthenticated(crypto: FingerprintManagerCompat.CryptoObject) {
                    viewModel.sendIntent(ViewModelIntent.ConfirmBiometricPromptEnabledSuccess(requireNotNull(crypto.cipher)))
                }
            })
    ...
    private fun showBiometricPromptForLogin(cipher: Cipher) {
        FingerprintAuthenticationDialogFragment().apply {
            ...
            setCallback(object : FingerprintAuthenticationDialogFragment.Callback {
                override fun onAuthenticated(crypto: FingerprintManagerCompat.CryptoObject) {
                    viewModel.sendIntent(ViewModelIntent.ShowAuthenticatedContent(requireNotNull(crypto.cipher)))
                }
            })
            show(supportFragmentManager, FingerprintAuthenticationDialogFragment::javaClass.name)
        }
